...
export const BottleListingCard = ({listing, isOwnListing = false, myPickupRequests = []}: BottleListingCardProps) => {
    ...
    const formattedDeadline = listing.deadline
        ? new Date(listing.deadline).toLocaleString('en-US', {
            month: 'short',
            day: 'numeric',
            hour: '2-digit',
            minute: '2-digit'
    ...
    const deadlineDate = listing.deadline ? new Date(listing.deadline) : null;
    ...
    const ownerPercentage = listing.sharePercentage ?? 50;
    ...
    const handleOfferPickup = () => {
        if (window.confirm(t('listing.confirmOffer', {count: listing.bottleCount, location: listing.location}))) {
    ...
    return (
        <Card
            className="hover:shadow-lg transition-all duration-300 border-green-100 hover:border-green-300 hover:scale-105">
            <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center space-x-2 min-w-0 flex-1">
                        <div
                            className="w-12 h-12 bg-gradient-to-r from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0">
                            {listing.bottleCount}
                        </div>
                        <div className="min-w-0 flex-1">
                            <h4 className="font-semibold text-gray-900 truncate"
                                title={listing.title || `${listing.bottleCount} Bottles Available`}>
                            ...
                            <div className="space-y-2">
                                <div className="bg-gray-50 rounded-lg p-3">
                                    <div className="flex items-center justify-between mb-2">
                                        <span
                                            className="text-sm font-medium text-gray-700">{t('listing.yourPickupRequest')}</span>
                                        <Badge
                                            className={
