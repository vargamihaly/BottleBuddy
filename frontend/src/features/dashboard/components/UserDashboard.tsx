...
import {ArrowLeft, ArrowUp, Loader2, MapPin, Plus, Recycle, Star, TrendingUp, Users} from "lucide-react";
import {useAuth} from "@/contexts/AuthContext";
import {useNavigate} from "react-router-dom";
import {useTranslation} from "react-i18next";
import {useUserActivities} from "@/features/notifications/hooks";
                    ...
                    <CardHeader>
                        <div className="flex items-center space-x-4">
                            {user.avatarUrl ? (
                                ...
                                <div
                                    className="w-16 h-16 bg-gradient-to-r from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white text-2xl font-bold">
                                ...
                                <CardDescription className="flex items-center space-x-2">
                                    {userStats.rating !== null && userStats.rating > 0 && (
                                        <>
                                            ...
                                            <span
                                                className="text-xs">({userStats.totalRatings} {t('userDashboard.profile.reviews')})</span>
                                            <span>•</span>
                                        </>
                                    )}
                                    {userStats.totalRatings === 0 && (
                                        <>
                                            <span
                                                className="text-xs text-gray-500">{t('userDashboard.profile.noRatings')}</span>
                                            <span>•</span>
                                        </>
                                    )}
                                ...
                                {recentActivity.map((activity) => (
                                    <div key={activity.id}
                                        ...
                                        <div className="flex items-center space-x-4 flex-1">
                                            <div
                                                className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center text-2xl flex-shrink-0">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {activity.rating && (
                                                    <div className="mt-2 flex items-center gap-2">
                                                        <span
                                                            className="text-yellow-500">{"⭐".repeat(activity.rating.value)}</span>
