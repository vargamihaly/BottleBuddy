...
import {useCreatePickupRequest} from "@/features/pickup-requests";
import {useMapViewData} from "@/features/bottle-listings";
...
export const MapView = ({listings, onBackToHome}: MapViewProps) => {
    ...
    const {hasActivePickupRequest, getPickupRequestStatus} = useMapViewData();
    ...
    const handleOfferPickup = (listing: BottleListingWithDistance) => {
        if (window.confirm(t("map.offerPickupConfirm", {
            count: listing.bottleCount,
            location: listing.locationAddress
        }))) {
